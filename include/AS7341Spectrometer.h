...
public:
  ...
  // Der AS7341-Rohkanal-Quirk (readAllChannels() liefert 12 Slots, Index 4/5 sind
  // Duplikate von F1-F4 aus dem zweiten Integrationszyklus; die echten F5-F8
  // liegen auf Slot 6-9) wird intern in measureRawSpectrum() aufgeloest -- niemand
  // ausserhalb dieser Klasse sieht die 12 Rohslots.
...
private:
  void computeVisReflectance(const std::vector<uint32_t>& raw, float R_vis[N_VIS]) const;
